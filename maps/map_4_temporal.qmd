...
Understanding temporal patterns reveals the rhythms of religious communication online, from daily posting cycles to annual liturgical seasons. This analysis examines over 600,000 posts from 2021 to 2025 to uncover when Croatian Catholics communicate, how patterns shift across platforms, and how the digital sphere responds to significant religious and secular events.
...
:
## Key Finding on Overall Trends

The time series reveals macro patterns in Croatian Catholic digital communication. Trend lines indicate whether the digital sphere is growing, stable, or contracting. Major peaks often correspond to significant religious events or news cycles.
:
...
:
## Key Finding on Liturgical Effects

The liturgical calendar shapes Croatian Catholic digital communication. Christmas and Easter periods show elevated activity, while specific feast days generate volume spikes. The effect sizes reveal which religious occasions generate the most digital engagement.
:
...
:
## Key Finding on Event Response

Different events generate distinct response patterns. Youth events (SHKM) produce concentrated spikes, while Vatican pronouncements may generate sustained discussion. Actor type response patterns reveal which segments of Catholic digital media engage with different event types.
:
...
:
## Key Finding on Platform Migration

Platform composition reveals structural shifts in Croatian Catholic digital communication. Growth or decline patterns indicate where future Catholic digital presence is likely to concentrate. Comparing growth rates helps identify emerging platforms and declining channels.
:
...
#| fig-height: 7
# Check if TIME column exists and create Hour column
if ("TIME" %in% names(dta)) {
  dta[, Hour := as.integer(substr(TIME, 1, 2))]
} else {
  # If TIME column doesn't exist, check for other time-related columns
  ...
  # Create a dummy Hour column if no time data available
...
# Also ensure other temporal columns exist - use lubridate::wday correctly
if (!"DOW" %in% names(dta) && "DATE" %in% names(dta)) {
  dta[, DOW := lubridate::wday(DATE, label = TRUE, abbr = FALSE)]
  dta[, DOW_num := lubridate::wday(DATE)]
}

# Now proceed with the hourly analysis
...
:
## Key Finding on Posting Rhythms

Posting rhythms reveal the temporal organization of Croatian Catholic digital communication. Institutional actors show office hour patterns while personal accounts post more in evenings and weekends. Sunday morning represents a distinctive Catholic rhythm potentially tied to Mass attendance and parish announcements.
:

# Analysis 4.6 Information Cascades

This analysis examines how content spreads across platforms and sources by tracking shared URLs.
...
ggplot(hourly_platform, aes(x = Hour, y = Share, color = SOURCE_TYPE)) +
  geom_line(linewidth = 1) +
  geom_point(data = hourly_platform[hourly_platform[, .I[which.max(Share)], by = SOURCE_TYPE]$V1],
             size = 3) +
  scale_color_manual(values = platform_colors) +
  scale_x_continuous(breaks = seq(0, 23, 4)) +
  labs(
    title = "Platform Posting Peaks Throughout the Day",
    subtitle = "Peak hours highlighted with points",
    x = "Hour",
    y = "Share of Platform's Daily Posts (%)",
    color = "Platform"
  ) +
  theme(legend.position = "right")
```
...
:
## Key Finding on Information Cascades

Platform activity correlations and temporal patterns reveal how content flows through the Croatian Catholic digital ecosystem. High correlations between platforms suggest synchronized communication patterns, potentially indicating that content originates on one platform and spreads to others.
:
...
# Check what data exists before creating the summary
cat("Sunday comparison data available:", nrow(sunday_comparison), "rows\n")
cat("Hour 10 data:", nrow(sunday_comparison[Hour == 10]), "rows\n")

# Create the summary with safe value extraction
tibble(
  Finding = c(
    "Corpus period",
  ...
  Value = c(
    ...
    # Safe extraction for Sunday index
...
The corpus reveals macro trends in Croatian Catholic digital communication across multiple years. Monthly and yearly aggregations show whether the digital sphere is expanding or contracting, while peak identification highlights periods of exceptional activity.

### Liturgical Calendar Effects

Croatian Catholic digital communication follows liturgical rhythms. The Christmas and Easter seasons show distinct patterns, and major feast days generate measurable volume increases. This confirms that religious calendar structures digital communication patterns.

### Event Response

Major events generate different response profiles depending on their nature. Youth events like SHKM produce concentrated spikes, Vatican pronouncements may generate sustained discussion, and political events reveal engagement from specific actor segments.

### Platform Migration

Platform composition shifts over time reveal structural changes in where Croatian Catholic digital communication occurs. Growth and decline patterns help identify which platforms are gaining importance and which are fading.

### Posting Rhythms

Micro temporal patterns reveal distinct rhythms between institutional and personal accounts. Institutional actors follow office hour patterns while individuals post more in evenings and weekends. Sunday morning represents a distinctive Catholic digital rhythm.

### Information Cascades

Platform activity correlations and content origin patterns suggest how information flows through the ecosystem. Understanding these patterns helps identify which platforms serve as originators versus amplifiers of Catholic digital content.
